...
In the Part 2 we mentioned the importance of studying the cell-type composition of a biological sample (i.e. bulk RNA sequencing deconvolution). The barcoding of RNA molecules according to the single cell origin gives higher information resolution. One of the benefits is a more direct estimation of cell-type composition. While for bulk RNA sequencing we need to use inference, with single-cell RNA sequencing the problem reduces to classification.   

Seurat is a very popular analysis framework for single cell RNA sequencing data [@butler2018integrating; @stuart2019comprehensive] .
...
```{r message=FALSE, warning=FALSE}
# Detach all previous packages - stackoverflow.com/questions/7505547/detach-all-packages-while-working-in-r
