...
vignette: >
  %\VignetteIndexEntry{A Tidy Transcriptomics introduction to RNA-Seq analyses}
...
knitr::include_graphics("../inst/vignettes/tidybulkpipeline-01.png")
```
...
counts_de_edger <- counts_filtered %>%
    test_differential_abundance(.formula=~dex)

# limma-voom
counts_de_voom <- counts_filtered %>%
 test_differential_abundance(.formula=~dex, method="limma_voom")

# DESeq2
counts_de_deseq2 <- counts_filtered %>%
    test_differential_abundance(.formula=~dex, method="DESeq2")
```
...
## Supplementary
Some things we didn't have time to cover in this workshop can be found in the [Supplementary material](https://stemangiola.github.io/rpharma2020_tidytranscriptomics/articles/supplementary.html).
