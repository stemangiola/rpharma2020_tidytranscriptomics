...
```{pool_1 class.source="pool"}
Pool: Will you code during this workshop?
```


```{pool_2 class.source="pool"}
Pool: Do you have experience with transcriptomic analyses?
```

```{pool_3 class.source="pool"}
Pool: Do you have experience with tidyverse?
```
...
```{pool_7 class.source="pool"}
Pool: What fraction of variance is explained by PC1?
```
...
```{pool_8 class.source="pool"}
Pool: How many differentially expressed genes are there for treated vs untreated (FDR < 0.05)?
```
...
```{pool_9 class.source="pool"}
Pool: What is the FBgn id of the 10th most differentially expressed gene (by smallest P value)?
```
...
Questions:  
1.1 What is the Fraction of Variance for PC1?  
1.2 How many differentially expressed genes are there for treated vs untreated (FDR < 0.05)?  
1.3 What is the FBgn id of the 10th most differentially expressed gene (by smallest P value)?
 ...
 * action="get" outputs the new information with the sample or transcript information, depending on what the analysis is
...
```{pool_4 class.source="pool"}
Pool: What is the method that detects more differentially abundant transcripts?
```
...
```{pool_5 class.source="pool"}
Pool: For the top ranked genes, DESeq2 estimates lower p.values compared to limma-voom. Roughly, around what point of gene rank the significance trend of DESeq2 crosses the one of limma-voom?
```
...
```{pool_6 class.source="pool"}
Pool: What is the most abundant cell type overall in BRCA samples?
```
