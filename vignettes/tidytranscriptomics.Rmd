...
```{r, out.width = "70%", warning=FALSE}
strip_chart %>% plotly::ggplotly(tooltip = c("label", "y"))
```
...
pasilla_de <- 
  ...
  # Convert input to tidybulk tibble
  tidybulk() %>%
...
We can also specify pre-selected gates with `gate_list`. This can be useful, for example, for reproducibility, if we want to store and reuse the coordinates of gates we've drawn.

```{r}
# a list containing two pre-selected gates
rpharma2020tidytranscriptomics::de_gate_gates
```
...
de_gate %>% 
  ...
  # Order transcripts (e.g. by smallest edgeR P value)
...
`tidybulk` allows for data nesting, using the `tidyr` utility `nest`. This is an extremely powerful tool as allows to easily perform analyses on subsets of the data.
...
Let's suppose we want to identify the markers (genes) that distinguish epithelial from endothelial cells, and we also want to then visualise their abundance across many cell types to understand their cell type specificity.

We'll use some example data that's in `cell_type_df`.

```{r}
# take a look
rpharma2020tidytranscriptomics::cell_type_df
```
We can check how many distinct cell types we have.

```{r}
rpharma2020tidytranscriptomics::cell_type_df %>%
    distinct(cell_type)
```
We will perform differential testing of epithelial versus endothelial cells, select some markers, and then visualise their abundance across all the cell types.
...
markers_df <- 
  ...
  # We test the two populations (endothelial vs epithelial) but we select data for all populations
  mutate(markers = map(
    comparison_data,
    ~ .x %>%
