...
Let's suppose we want to identify the markers that distinguish epithelial from endothelial cells, but we want to then visualising those genes across many cell types to understand the background transcription.
...
- `tidyseurat` is a invisible layer that operates on `Seurat` object and allows to visualise and manipulate data as if it were a tidy data frame.
- `tidyseurat` object are `Seurat object` so they can be used with any `Seurat` compatible method
